...
setDeskEnv(){
	...
	sudo sed -i '/^#greeter-hider-user=/s/#//' /etc/lightdm/lightdm.conf
	sudo wget "$repository/back.jpg" -O /usr/share/pixmaps/back.jpg 2>/dev/null
	sudo wget "$repository/key" -O /etc/X11/xorg.conf.d/10-evdev.conf 2>/dev/null
